...
"""
该部分对数据集进行了比较粗浅的可视化分析。
用图表的显示方式，直观的展示了是否患心脏病与各个变量之间的关系。
"""
# 使中文能够在画出的图中正常显示
...
data = pd.read_csv('D:\编程文件夹\《大数据处理课程设计》\Heart-Disease-Analysis\数据集\heart_disease_preprocessed.csv')

# 画图
# 图0 每两个变量之间的相关性热力图
data0.corr()
plt.figure(figsize=(10, 10))
sns.heatmap(data0.corr(), annot=True, square=True)
plt.show()
